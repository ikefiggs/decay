...
// this npc is the fast zombie from hl2 except without the jumping attack. able to climb the climb nodes.
...
#define HOODIE_IDLE_PITCH			35
#define HOODIE_MIN_PITCH			70
#define HOODIE_MAX_PITCH			130
#define HOODIE_SOUND_UPDATE_FREQ	0.5

#define HOODIE_MAXLEAP_Z		128

#define HOODIE_EXCITE_DIST 480.0

#define HOODIE_BASE_FREQ 1.5

// If flying at an enemy, and this close or closer, start playing the maul animation!!
#define HOODIE_MAUL_RANGE	300

#ifdef HL2_EPISODIC

int AE_PASSENGER_PHYSICS_PUSHING;
int AE_HOODIE_VEHICLE_LEAP;
int AE_HOODIE_VEHICLE_SS_DIE;	// Killed while doing scripted sequence on vehicle

#endif // HL2_EPISODIC

enum
{
	COND_HOODIE_CLIMB_TOUCH = LAST_BASE_ZOMBIE_CONDITION,
};

envelopePoint_t envHoodieVolumeJump[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieVolumePain[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieInverseVolumePain[] =
{
	{ 0.0f, 0.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieVolumeJumpPostApex[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieVolumeClimb[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieMoanVolumeFast[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieMoanVolume[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieFootstepVolume[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envHoodieVolumeFrenzy[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
//=========================================================
int AE_HOODIE_LEAP;
int AE_HOODIE_GALLOP_LEFT;
int AE_HOODIE_GALLOP_RIGHT;
int AE_HOODIE_CLIMB_LEFT;
int AE_HOODIE_CLIMB_RIGHT;
...
enum
{
	TASK_HOODIE_DO_ATTACK = LAST_SHARED_TASK + 100,	// again, my !!!HACKHACK
	TASK_HOODIE_LAND_RECOVER,
	TASK_HOODIE_UNSTICK_JUMP,
	TASK_HOODIE_JUMP_BACK,
	TASK_HOODIE_VERIFY_ATTACK,
};

//=========================================================
// activities
//=========================================================
int ACT_HOODIE_LEAP_SOAR;
int ACT_HOODIE_LEAP_STRIKE;
int ACT_HOODIE_LAND_RIGHT;
int ACT_HOODIE_LAND_LEFT;
int ACT_HOODIE_FRENZY;
int ACT_HOODIE_BIG_SLASH;
...
enum
{
	SCHED_HOODIE_RANGE_ATTACK1 = LAST_SHARED_SCHEDULE + 100, // hack to get past the base zombie's schedules
	SCHED_HOODIE_UNSTICK_JUMP,
	SCHED_HOODIE_CLIMBING_UNSTICK_JUMP,
	SCHED_HOODIE_MELEE_ATTACK1,
	SCHED_HOODIE_TORSO_MELEE_ATTACK1,
};
...
//=========================================================
//=========================================================
class CHoodie : public CNPC_BaseZombie		//it is a zombie
{
	DECLARE_CLASS(CHoodie, CNPC_BaseZombie);  //it is a zombie

public:
	void Spawn(void);
	void Precache(void);

	void SetZombieModel(void);			//set model 
...
LINK_ENTITY_TO_CLASS(npc_hoodie, CHoodie);
LINK_ENTITY_TO_CLASS(npc_fastzombie_torso, CHoodie);		// torso


BEGIN_DATADESC(CHoodie)
...
const char *CHoodie::pMoanSounds[] =
{
...
//
//-----------------------------------------------------------------------------
void CHoodie::Precache(void)
{
	PrecacheModel("models/alien/sprinter.mdl");		//model precache
#ifdef HL2_EPISODIC
...
//---------------------------------------------------------
//---------------------------------------------------------
void CHoodie::OnScheduleChange(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CHoodie::SelectSchedule(void)
{
	...
	if (HasCondition(COND_ZOMBIE_RELEASECRAB))			//basezombie crab
	{
		// Death waits for no man. Or zombie. Or something.
		return SCHED_ZOMBIE_RELEASECRAB;
	}

	if (HasCondition(COND_HOODIE_CLIMB_TOUCH))
	{
		return SCHED_HOODIE_UNSTICK_JUMP;
	}
...
#ifdef DEBUG_ZOMBIES			//zombie wandering 
			DevMsg("Wandering\n");
#endif

			// Just lost track of our enemy. 
			// Wander around a bit so we don't look like a dingus.
			return SCHED_ZOMBIE_WANDER_MEDIUM;		//wandering behavior .h
		}
		break;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHoodie::PrescheduleThink(void)
{
	...
	if (m_pMoanSound && gpGlobals->curtime > m_flTimeUpdateSound)
	{
		...
		{
			// Calm down!
			flDistNoBBox = HOODIE_EXCITE_DIST;
			m_flTimeUpdateSound += 1.0;
		}

		if (flDistNoBBox >= HOODIE_EXCITE_DIST && m_flDistFactor != 1.0)
		{
			// Go back to normal pitch.
			m_flDistFactor = 1.0;

			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, HOODIE_IDLE_PITCH, HOODIE_SOUND_UPDATE_FREQ);
		}
		else if (flDistNoBBox < HOODIE_EXCITE_DIST)
		{
			// Zombie is close! Recalculate pitch.
			int iPitch;

			m_flDistFactor = MIN(1.0, 1 - flDistNoBBox / HOODIE_EXCITE_DIST);
			iPitch = HOODIE_MIN_PITCH + ((HOODIE_MAX_PITCH - HOODIE_MIN_PITCH) * m_flDistFactor);
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, iPitch, HOODIE_SOUND_UPDATE_FREQ);
		}

		m_flTimeUpdateSound = gpGlobals->curtime + HOODIE_SOUND_UPDATE_FREQ;
	}
	...
	if (IsCurSchedule(SCHED_HOODIE_RANGE_ATTACK1, false))
	{
...
//
//-----------------------------------------------------------------------------
void CHoodie::SoundInit(void)
{
	if (!m_pMoanSound)
	{
		// !!!HACKHACK - kickstart the moan sound. (sjb)
		MoanSound(envHoodieMoanVolume, ARRAYSIZE(envHoodieMoanVolume));
...
//-----------------------------------------------------------------------------
void CHoodie::SetIdleSoundState(void)
{
	// Main looping sound
	if (m_pMoanSound)
	{
		ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, HOODIE_IDLE_PITCH, 1.0);
...
//-----------------------------------------------------------------------------
void CHoodie::SetAngrySoundState(void)
{
	...
	// Main looping sound
	ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, HOODIE_MIN_PITCH, 0.5);
...
//
//-----------------------------------------------------------------------------
void CHoodie::Spawn(void)
{
	Precache();

	m_fJustJumped = false;

	m_fIsTorso = m_fIsHeadless = true;		// headless

	if (FClassnameIs(this, "npc_hoodie"))
	{
		m_fIsTorso = false;
	}
	else
...
//-----------------------------------------------------------------------------
const char *CHoodie::GetHeadcrabClassname(void)
{
	return "npc_headcrab_fast";
}

const char *CHoodie::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CHoodie::MaxYawSpeed(void)
{
	switch (GetActivity())
	{
	case ACT_TURN_LEFT:
...
//
//-----------------------------------------------------------------------------
void CHoodie::SetZombieModel(void)
{
	...
	{
		SetModel("models/alien/sprinter.mdl");
...
//-----------------------------------------------------------------------------
const char *CHoodie::GetLegsModel(void)
{
	return s_pLegsModel;
}

const char *CHoodie::GetTorsoModel(void)
{
...
//
//-----------------------------------------------------------------------------
int CHoodie::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (!GetEnemy())
	{
		return COND_NONE;
...
//-----------------------------------------------------------------------------
const char *CHoodie::GetMoanSound(int nSound)
{
...
//-----------------------------------------------------------------------------
void CHoodie::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CHoodie::AttackHitSound(void)
{
...
//-----------------------------------------------------------------------------
void CHoodie::AttackMissSound(void)
{
...
//-----------------------------------------------------------------------------
//void CHoodie::LeapAttackSound(void)
//{
	//EmitSound("NPC_FastZombie.LeapAttack");
//}
...
//-----------------------------------------------------------------------------
void CHoodie::PainSound(const CTakeDamageInfo &info)
{
	if (m_pLayer2)
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envHoodieVolumePain, ARRAYSIZE(envHoodieVolumePain));
	if (m_pMoanSound)
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, envHoodieInverseVolumePain, ARRAYSIZE(envHoodieInverseVolumePain));
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHoodie::DeathSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//#define HOODIE_MINLEAP			200
//#define HOODIE_MAXLEAP			300
//float CHoodie::InnateRange1MaxRange(void)
//{
	//return HOODIE_MAXLEAP;
//}
...
//
//-----------------------------------------------------------------------------
//int CHoodie::RangeAttack1Conditions(float flDot, float flDist)
//{
...
//
//-----------------------------------------------------------------------------
void CHoodie::HandleAnimEvent(animevent_t *pEvent)
{
	if (pEvent->event == AE_HOODIE_CLIMB_LEFT || pEvent->event == AE_HOODIE_CLIMB_RIGHT)
	{
		if (++m_iClimbCount % 3 == 0)
		{
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pLayer2, random->RandomFloat(100, 150), 0.0);
			ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envHoodieVolumeClimb, ARRAYSIZE(envHoodieVolumeClimb));
		}

		return;
	}

	//if (pEvent->event == AE_HOODIE_LEAP)
	//{
		//LeapAttack();
		//return;
	//}

	if (pEvent->event == AE_HOODIE_GALLOP_LEFT)
	{
		EmitSound("NPC_FastZombie.GallopLeft");
		return;
	}

	if (pEvent->event == AE_HOODIE_GALLOP_RIGHT)
	{
		EmitSound("NPC_FastZombie.GallopRight");
		return;
	}

	if (pEvent->event == AE_ZOMBIE_ATTACK_RIGHT)		//zombie .h
	{
		Vector right;
	...
	if (pEvent->event == AE_ZOMBIE_ATTACK_LEFT)		// zombie .h
	{
		Vector right;
		AngleVectors(GetLocalAngles(), NULL, &right, NULL);
		right = right * 50;
	...
	// Do the leap attack
	if (pEvent->event == AE_HOODIE_VEHICLE_LEAP)
	{
		//VehicleLeapAttack();
		return;
	}
	...
	if (pEvent->event == AE_HOODIE_VEHICLE_SS_DIE)
	{
		if (IsInAVehicle())
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHoodie::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_HOODIE_VERIFY_ATTACK:
	...
	case TASK_HOODIE_LAND_RECOVER:
	{
		...
		if (flDeltaYaw < 0)
		{
			SetIdealActivity((Activity)ACT_HOODIE_LAND_RIGHT);
		}
		else
		{
			SetIdealActivity((Activity)ACT_HOODIE_LAND_LEFT);
		}
	...
	case TASK_RANGE_ATTACK1:
		...
		SetTouch(&CHoodie::LeapAttackTouch);
		break;

	case TASK_HOODIE_DO_ATTACK:
		SetActivity((Activity)ACT_HOODIE_LEAP_SOAR);
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHoodie::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_HOODIE_JUMP_BACK:
...
//---------------------------------------------------------
//---------------------------------------------------------
int CHoodie::TranslateSchedule(int scheduleType)
{
	switch (scheduleType)
	{
	case SCHED_RANGE_ATTACK1:
	{
		...
		return SCHED_HOODIE_RANGE_ATTACK1;
	}
	break;
	...
	case SCHED_MELEE_ATTACK1:
		if (m_fIsTorso == true)
		{
			return SCHED_HOODIE_TORSO_MELEE_ATTACK1;
		}
		else
		{
			return SCHED_HOODIE_MELEE_ATTACK1;
		}
		break;

	case SCHED_HOODIE_UNSTICK_JUMP:
		if (GetActivity() == ACT_CLIMB_UP || GetActivity() == ACT_CLIMB_DOWN || GetActivity() == ACT_CLIMB_DISMOUNT)
		{
			return SCHED_HOODIE_CLIMBING_UNSTICK_JUMP;
		}
		else
		{
			return SCHED_HOODIE_UNSTICK_JUMP;
...
//---------------------------------------------------------
//---------------------------------------------------------
Activity CHoodie::NPC_TranslateActivity(Activity baseAct)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CHoodie::LeapAttackTouch(CBaseEntity *pOther)
{
	if (!pOther->IsSolid())
	{
...
//-----------------------------------------------------------------------------
void CHoodie::ClimbTouch(CBaseEntity *pOther)
{
	if (pOther->IsPlayer())
	{
		...
		{
			SetCondition(COND_HOODIE_CLIMB_TOUCH);
		}
...
//-----------------------------------------------------------------------------
void CHoodie::StopLoopingSounds(void)
{
	if (m_pMoanSound)
	{
...
//-----------------------------------------------------------------------------
void CHoodie::BecomeTorso(const Vector &vecTorsoForce, const Vector &vecLegsForce)
{
...
// Output :
//-----------------------------------------------------------------------------
bool CHoodie::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
bool CHoodie::MovementCost(int moveType, const Vector &vecStart, const Vector &vecEnd, float *pCost)
{
...
bool CHoodie::ShouldFailNav(bool bMovementFailed)
{
...
//---------------------------------------------------------
void CHoodie::OnNavJumpHitApex(void)
{
...
//			jump state.
//---------------------------------------------------------
void CHoodie::OnChangeActivity(Activity NewActivity)
{
	if (NewActivity == ACT_HOODIE_FRENZY)
	{
		// Scream!!!!
	...
	else if (GetActivity() == ACT_GLIDE)
	{
		...
		if (m_pMoanSound)
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, HOODIE_MIN_PITCH, 0.3);
	}
...
// 
//=========================================================
int CHoodie::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	if (m_fJustJumped)
	{
...
//=========================================================
void CHoodie::BeginNavJump(void)
{
	m_fIsNavJumping = true;
	m_fHitApex = false;

	ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envHoodieVolumeJump, ARRAYSIZE(envHoodieVolumeJump));
}

//=========================================================
// 
//=========================================================
void CHoodie::EndNavJump(void)
{
...
// 
//=========================================================
void CHoodie::BeginAttackJump(void)
{
...
// 
//=========================================================
void CHoodie::EndAttackJump(void)
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHoodie::BuildScheduleTestBits(void)
{
	...
	if (GetActivity() == ACT_CLIMB_UP || GetActivity() == ACT_CLIMB_DOWN || GetActivity() == ACT_CLIMB_DISMOUNT)
	{
		SetCustomInterruptCondition(COND_HOODIE_CLIMB_TOUCH);
	}
	else
	{
		ClearCustomInterruptCondition(COND_HOODIE_CLIMB_TOUCH);
	}
}
...
// 
//=========================================================
void CHoodie::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHoodie::Event_Killed(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CHoodie::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
	if (m_fIsTorso)
	{
		// Already split.
...
//-----------------------------------------------------------------------------
bool CHoodie::CreateBehaviors(void)
{
...
//-----------------------------------------------------------------------------
void CHoodie::InputAttachToVehicle(inputdata_t &inputdata)
{
	// Interrupt us
...
//-----------------------------------------------------------------------------
void CHoodie::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
...
//-----------------------------------------------------------------------------
//void CHoodie::VehicleLeapAttack(void)
//{
	//CBaseEntity *pEnemy = GetEnemy();
	//if (pEnemy == NULL)
		//return;
	...
	//SetTouch(&CHoodie::VehicleLeapAttackTouch);
//}
...
//-----------------------------------------------------------------------------
bool CHoodie::CanEnterVehicle(CPropJeepEpisodic *pVehicle)
{
	if (pVehicle == NULL)
		return false;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
//void CHoodie::VehicleLeapAttackTouch(CBaseEntity *pOther)
//{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CHoodie::IsInAVehicle(void)
{
...
//-----------------------------------------------------------------------------
void CHoodie::UpdateEfficiency(bool bInPVS)
{
...
AI_BEGIN_CUSTOM_NPC(npc_hoodie, CHoodie)

DECLARE_ACTIVITY(ACT_HOODIE_LEAP_SOAR)
DECLARE_ACTIVITY(ACT_HOODIE_LEAP_STRIKE)
DECLARE_ACTIVITY(ACT_HOODIE_LAND_RIGHT)
DECLARE_ACTIVITY(ACT_HOODIE_LAND_LEFT)
DECLARE_ACTIVITY(ACT_HOODIE_FRENZY)
DECLARE_ACTIVITY(ACT_HOODIE_BIG_SLASH)

DECLARE_TASK(TASK_HOODIE_DO_ATTACK)
DECLARE_TASK(TASK_HOODIE_LAND_RECOVER)
DECLARE_TASK(TASK_HOODIE_UNSTICK_JUMP)
DECLARE_TASK(TASK_HOODIE_JUMP_BACK)
DECLARE_TASK(TASK_HOODIE_VERIFY_ATTACK)

DECLARE_CONDITION(COND_HOODIE_CLIMB_TOUCH)

//Adrian: events go here
DECLARE_ANIMEVENT(AE_HOODIE_LEAP)
DECLARE_ANIMEVENT(AE_HOODIE_GALLOP_LEFT)
DECLARE_ANIMEVENT(AE_HOODIE_GALLOP_RIGHT)
DECLARE_ANIMEVENT(AE_HOODIE_CLIMB_LEFT)
DECLARE_ANIMEVENT(AE_HOODIE_CLIMB_RIGHT)

#ifdef HL2_EPISODIC
// FIXME: Move!
DECLARE_ANIMEVENT(AE_PASSENGER_PHYSICS_PUSHING)
DECLARE_ANIMEVENT(AE_HOODIE_VEHICLE_LEAP)
DECLARE_ANIMEVENT(AE_HOODIE_VEHICLE_SS_DIE)
#endif	// HL2_EPISODIC
...
DEFINE_SCHEDULE
(
SCHED_HOODIE_RANGE_ATTACK1,

"	Tasks"
"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_RANGE_ATTACK1"
"		TASK_SET_ACTIVITY				ACTIVITY:ACT_HOODIE_LEAP_STRIKE"
"		TASK_RANGE_ATTACK1				0"
"		TASK_WAIT						0.1"
"		TASK_HOODIE_LAND_RECOVER	0" // essentially just figure out which way to turn.
"		TASK_FACE_ENEMY					0"
"	"
"	Interrupts"
)
...
"		TASK_MELEE_ATTACK1				0"
"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_HOODIE_FRENZY"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CHASE_ENEMY"
"		TASK_HOODIE_VERIFY_ATTACK	0"
"		TASK_PLAY_SEQUENCE_FACE_ENEMY	ACTIVITY:ACT_HOODIE_BIG_SLASH"
